...
class InterpreteurDeMessage:
    ...
    def distribution(self,message:str):
        quoi, reste = self.quoi(message) 
        if self.est_ce_une_commandeq(quoi): 
            self.commande(quoi, reste)           
        elif self.est_ce_un_numeroq(quoi):
            self.pour_Vigogne(quoi, reste)
        else:
            print(f"Erreur js ne sais pas quoi faire de {quoi}")
